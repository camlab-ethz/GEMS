...
num_atomfeatures = 40
num_edgefeatures = 17
output_folder = f'/data/grbv/PDBbind/DTI_4a2/input_graphs_{embedding_descriptor}/'


# GET THE PREPROCESSED DATA
# -------------------------------------------------------------------------------
# Generate a lists of all protein-ligand complexes, the corresponding folder path and protein_dictionary paths
input_data_dir = '/data/grbv/PDBbind/DTI_input_data_processed'
...
for complex_id, folder_path, protein_path, ligand_path in zip(complexes, folder_paths, protein_paths, ligand_paths):
    ...
    try:
        connections_dict = load_object( os.path.join(folder_path, f'{complex_id}_connections_starline5A.pkl') )
    ...
    master_lig = [[i for i in range(x_lig.shape[0])],
