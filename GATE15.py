...
GATE15a: Initial Node and Global transformation
GATE15b: Initial Node and Global transformation without dropout
GATE15c: Initial Node and Global transformation with dropout 0.1
...
class GATE15a(nn.Module):
    def __init__(self, dropout_prob, in_channels, edge_dim, conv_dropout_prob):
        super(GATE15a, self).__init__()

        self.NodeTransform = FeatureTransformMLP(in_channels, 256, 256, dropout=dropout_prob)
        self.GlobalTransform = FeatureTransformMLP(384, 256, 256, dropout=dropout_prob)
        
        self.layer1 = self.build_layer(node_f=256, edge_f=edge_dim, node_f_hidden=256, edge_f_hidden=64, node_f_out=256, edge_f_out=128, global_f=256, residuals=False, dropout=conv_dropout_prob)
    ...
    def forward(self, graphbatch):
        ...
        u = self.GlobalTransform(graphbatch.lig_emb)

        x, edge_attr, u = self.layer1(x, edge_index, graphbatch.edge_attr, u, batch=graphbatch.batch)
        x = self.node_bn1(x)
...
class GATE15b(nn.Module):
    def __init__(self, dropout_prob, in_channels, edge_dim, conv_dropout_prob):
        super(GATE15b, self).__init__()

        self.NodeTransform = FeatureTransformMLP(in_channels, 256, 256, dropout=0)
        self.GlobalTransform = FeatureTransformMLP(384, 256, 256, dropout=0)
...
class GATE15c(nn.Module):
    def __init__(self, dropout_prob, in_channels, edge_dim, conv_dropout_prob):
        super(GATE15c, self).__init__()

        self.NodeTransform = FeatureTransformMLP(in_channels, 256, 256, dropout=0.1)
        self.GlobalTransform = FeatureTransformMLP(384, 256, 256, dropout=0.1)
