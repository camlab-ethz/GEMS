...
def read_pdb_codes(filename):
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            columns = line.split()
            # The first column contains the PDB code
            pdb_code = columns[0]
            pdb_codes.append(pdb_code)
    return pdb_codes

filename = '/cluster/work/math/dagraber/DTI/INDEX_general_PL_data.2016'
v2016 = read_pdb_codes(filename)
...
distance_matrix = 'pairwise_similarity_matrix/pairwise_similarity_casf.hdf5'
...
log_file = open('dataset_filtering/v2016_test/training_set_filtering.log', 'w')

# Get the indexes and names from json file
with open('pairwise_similarity_matrix/pairwise_similarity_complexes.json', 'r') as f:
...
not_in_v2016 = np.array([False if complex in v2016 else True for complex in complexes])
...
for test_idx, test_complex in test_set:
    ...
    # Set all the similarities to zero if the complex is not in v2016
    metrics[not_in_v2016, :] = 0
...
with open('dataset_filtering/v2016_test/PDBbind_c7_data_split.json', 'w', encoding='utf-8') as json_file:
    json.dump(split_dict, json_file, ensure_ascii=False, indent=4)

with open('dataset_filtering/v2016_test/train_test_similarities_casf2016.json', 'w', encoding='utf-8') as json_file:
    json.dump(train_test_sims_casf2016, json_file, ensure_ascii=False, indent=4)

with open('dataset_filtering/v2016_test/train_test_similarities_casf2016_n.json', 'w', encoding='utf-8') as json_file:
    json.dump(train_test_sims_casf2016_n, json_file, ensure_ascii=False, indent=4)

with open('dataset_filtering/v2016_test/train_test_similarities_casf2013.json', 'w', encoding='utf-8') as json_file:
    json.dump(train_test_sims_casf2013, json_file, ensure_ascii=False, indent=4)

with open('dataset_filtering/v2016_test/train_test_similarities_casf2013_n.json', 'w', encoding='utf-8') as json_file:
