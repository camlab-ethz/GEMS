...
folder_path = f'/data/grbv/PDBbind/DTI_5_{curation}'




# Generate lists of complexes in the test dataset and in the refined dataset
...
test_complexes = casf2016_complexes
...
def analyse_cluster(name, data):
    ...
    try:
        ...
        # in the cluster and if yes, remove all training set complexes with similar affinity
        while len(data) > 0:
    ...
    distance_matrix.to_csv(f'/data/grbv/PDBbind/DTI_5_{curation}/clusters/{name}_distance_matrix.csv')
    ...
    while len(conflicts) > 0:
        ...
        # Remove complexes of the general set with higher priority
...
with open(clustering_output, 'r', newline='') as infile, open(f'dataset_cleaning_DTI5_{curation}.csv', 'w', newline='') as outfile:
