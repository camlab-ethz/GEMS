...
def analyse_cluster(name, data):
    ...
    #--------------------------------------------------------------------------------------------------
    db_file = f'/data/grbv/PDBbind/DTI_5_1/clusters/{name}_db_smiles.h5'
    ...
    for i in range(N):
        ...
        query = smiles_dict[data[i][0]] # Get smiles of first complex with its id

        try:
            results = fpe.tversky(query, 0.5, 0.7, 0.3, n_workers=1)
    ...
    distance_matrix.to_csv(f'/data/grbv/PDBbind/DTI5_clusters/{name}_distance_matrix.csv')
    ...
    conflicting = [word for tuple in conflicts for word in tuple]
    ...
    for comp in conflicting:
        if comp in test_complexes:
            ...
            to_remove = list(set([y for x, y in similarities if x == comp] + [x for x, y in similarities if y == comp]))
    ...
    while len(conflicts) > 0:
        ...
        reasons.append(list(set([y for x, y in similarities if x == most_frequent] + [x for x, y in similarities if y == most_frequent])))
...
with open(clustering_output, 'r', newline='') as infile, open('dataset_cleaning_DTI5_1.csv', 'w', newline='') as outfile:
            ...
            if len(current) > 1: 
                ...
                for r, reason in zip(removed, reasons):
                    ...
                    if not isinstance(reason, list):
                        ...
                        writer.writerow([f'{name} cluster: Removed {r} due to similarities with {reason}'])     
