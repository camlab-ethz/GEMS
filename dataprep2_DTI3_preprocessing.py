...
output_data_dir = f'/data/grbv/PDBbind/{experiment_name}/input_data_processed/'
affinity_dict = load_object(f'/data/grbv/PDBbind/{experiment_name}/v2020_general_affinity_dict.pkl')
...
log_file_path = os.path.join(log_folder, "preprocessing.txt")
...
for protein_id, ligand_path, protein_path in zip(protein_ids, ligand_paths, protein_paths):
    ...
    close = pairwise_distances <= max_len
    ...
    #------------------------------------------------------------------
    selected_atoms = connect_to_accessible_aa(pos, sel, max_len=max_len)
    #------------------------------------------------------------------
    ...
    filepath = os.path.join(save_dir, f'{protein_id}_connections.pkl')
