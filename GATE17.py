...
NEW: global feature COMPUTATION IS FLEXIBLE (Can become larger)
NEW: MLPs only have one linear layer
...
GATE17a: MLPs only have one linear layer
...
class FeatureTransform(nn.Module):
    def __init__(self, node_feature_dim, out_dim, dropout):
        super(FeatureTransform, self).__init__()
        self.dropout_layer = nn.Dropout(dropout)
        self.linear = nn.Sequential(
            nn.Linear(node_feature_dim, out_dim),
            nn.ReLU()
        )
...
class GATE17a(nn.Module):
    def __init__(self, dropout_prob, in_channels, edge_dim, conv_dropout_prob):
        super(GATE17a, self).__init__()

        self.NodeTransform = FeatureTransform(in_channels, 256, dropout=dropout_prob)
